...
export function LiveKitChatScreen({ onBack, onError, alarmId, autoStart = false, alarmTitle, alarmGoal }: Props) {
  ...
  const startSession = useCallback(async () => {
    ...
    try {
      ...
      const token = await requestLiveKitToken(config.apiBaseUrl, {
        ...
        alarmId,
        sessionContext: { location: config.location, coachName: config.coachName },
      });
  ...
  }, [alarmId, attachAudio, cleanupAudio, config, onError, phase]);
  ...
  return <CoachLayout config={config} phase={phase} agentState={agentState} agentConnected={agentConnected} transcriptLines={transcriptLines} error={error} microphoneEnabled={microphoneEnabled} alarmTitle={alarmTitle} alarmGoal={alarmGoal} onBack={handleBack} onEnd={() => { stopSession("ended"); onBack?.(); }} onMicrophone={phase === "active" ? toggleMicrophone : startSession} />;
}

function CoachLayout({ config, phase, agentState, agentConnected, transcriptLines, error, microphoneEnabled, alarmTitle, alarmGoal, onBack, onEnd, onMicrophone }: { config: ChatConfig; phase: SessionPhase; agentState: AgentState; agentConnected: boolean; transcriptLines: string[]; error: string | null; microphoneEnabled: boolean; alarmTitle?: string; alarmGoal?: string; onBack: () => void; onEnd: () => void; onMicrophone: () => void }) {
...
function readChatConfig(): ChatConfig {
  ...
  return { apiBaseUrl: String(extra.apiBaseUrl ?? "http://127.0.0.1:8000"), roomPrefix: String(extra.livekitRoomName ?? "chronos-coach"), location: String(extra.chatLocation ?? "徐汇体育馆游泳"), coachName: String(extra.chatCoachName ?? "Chronos教练"), statusText: String(extra.chatStatusText ?? "（更新了记忆）") };
}
